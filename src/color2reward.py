...
def color_detect(img): # assign reward value: -1
    ...
    # yellow mask
    hsv_min_yellow = np.array([31, 252, 251])  # actually yellow
    hsv_max_yellow = np.array([31, 255, 255])  # actually yellow
    ...
    hsv_max_red = np.array([179, 255, 255])
    ...
    return mask1_red+mask2_red, mask_lgray, mask_dgray, mask1_spink+mask2_spink, mask_yellow
...
def image2reward(image_file, matvals=True):
    ...
    mask_red, mask_lgray, mask_dgray, mask_spink, mask_yellow = color_detect(img)
    ...
    for i in range(index_yellow.shape[1]):
        rewardMatrix[index_yellow[0][i]][index_yellow[1][i]] = 5
