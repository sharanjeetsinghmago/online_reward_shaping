...
def a_star_planning(sx, sy, gx, gy, rewardMatrix, greso, rs, xwidth, minx, miny, maxx, maxy):
    ...
    while (True):
        ...
        c_id = min(openset,
                   key=lambda o: openset[o].cost + rewardWeight * costMatrix[int(o)] + 3 * calc_heuristic(ngoal,
                                                                                                          openset[o]))
...
def main():
    ...
    imgstars = cv2.imread('1001atacamastarsall.png')
    imgorig = cv2.imread('atacamaTexture1001.png')
    imgstars_cropped = imgstars[0:99][0:99][:]
    imgorig_cropped = imgorig[0:99][0:99][:]
    ...
    rx, ry, costMatrix, accumReward = a_star_planning(sx, sy, gx, gy, rewardMatrix, greso, rs, xwidth, minx, miny, maxx,
                                                      maxy)
    ...
    im1 = ax1.pcolor(xgrid_show, ygrid_show, rewardMatrix, cmap='plasma', vmin=-5.0, vmax=10.0)
    ...
    im2 = ax2.pcolor(xgrid_show, ygrid_show, dispcostMatrix, cmap='viridis', vmin=0, vmax=15.0)
